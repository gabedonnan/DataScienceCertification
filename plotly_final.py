...
app.layout = html.Div(children=[html.H1('SpaceX Launch Records Dashboard',
                                ...
                                dcc.Dropdown(
                                    options=[
                                        ...
                                        value="ALL",
                                        searchable=True,
                                        placeholder="Choose landing site",
                                        id="site-dropdown"),
...
def get_pie_chart(entered_site):
    filtered = spacex_df
    if entered_site == "ALL":
        fig = px.pie(filtered, values="class", names='Launch Site', title="Total success launches by site")
        return fig
    elif entered_site == "CCAFS LC-40":
        filtered = filtered[filtered["Launch Site"] == "CCAFS LC-40"].reset_index()
        labels = filtered["class"].value_counts().index
        values = filtered["class"].value_counts().values
        fig = px.pie(filtered, values=values, names=labels, title="Total success launches for CCAFS LC-40")
        return fig
    elif entered_site == "VAFB SLC-4E":
        filtered = filtered[filtered["Launch Site"] == "VAFB SLC-4E"].reset_index()
        labels = filtered["class"].value_counts().index
        values = filtered["class"].value_counts().values
        fig = px.pie(filtered, values=values, names=labels, title="Total success launches for VAFB SLC-4E")
        return fig
    elif entered_site == "KSC LC-39A":
        filtered = filtered[filtered["Launch Site"] == "KSC LC-39A"].reset_index()
        labels = filtered["class"].value_counts().index
        values = filtered["class"].value_counts().values
        fig = px.pie(filtered, values=values, names=labels, title="Total success launches for KSC LC-39A")
        return fig
    elif entered_site == "CCAFS SLC-40":
        filtered = filtered[filtered["Launch Site"] == "CCAFS SLC-40"].reset_index()
        labels = filtered["class"].value_counts().index
        values = filtered["class"].value_counts().values
        fig = px.pie(filtered, values=values, names=labels, title="Total success launches for CCAFS SLC-40")
        return fig
